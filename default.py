...
fpt=xbmcaddon.Addon(id='plugin.video.fptt3')
pluginhandle = int(sys.argv[1])
user_agent = 'Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:10.0a1) Gecko/20111029 Firefox/10.0a1'

# this is only in here for xbmc v.10 and below, because they have python 2.4 internally and we needed any() which didn't appear until py2.5
#def any(iterable):
...
favPath=os.path.join(path,'favtv.txt')

#hack for python 2.4 to make sure favtv.txt is available, for some reason 'a+' wasn't creating the file on osx using py2.4
...
def CATS():
    addDir('Favorite TV Shows','http://www.fastpasstv.ms/',20,'')
...
def ADDFAVS(url,name):
    read = open(favPath, 'a+')
    read.write('"'+url+'\n')
    xbmc.executebuiltin("XBMC.Notification(Added as favorite!,,3000)")
    read.close()

def REMFAVS(url,name):
    murl=re.compile('(.+?)\|').findall(url)
    xbmc.executebuiltin("XBMC.Notification(Removed as favorite!,,3000)")
    read = open(favPath, 'r')
...
def TVLST(url,name):
    ...
    name2='Add as Favorite'
    name3='Remove as Favorite'
    try:
        read = open(favPath, 'r')
    except IOError:
        read = open(favPath, 'a+')
